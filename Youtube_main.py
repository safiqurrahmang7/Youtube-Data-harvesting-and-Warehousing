...
st.markdown(
    """
    <h1>
        <img src="https://upload.wikimedia.org/wikipedia/commons/4/42/YouTube_icon_%282013-2017%29.png" alt="YouTube Icon" width="50">
...
with st.sidebar:
    select = option_menu('Main Menu',("Data_Handling", "Data Migration","JSON and Table","Data Visualization"))

if select == "Data_Handling":
...
if select ==  "Data Migration":
    ...
    db_chnl = cursor.fetchall()
    db_chnl1 = [item[0] for item in db_chnl]
    ...
    uniq_channel = [channel for channel in all_channels if channel not in db_chnl1]
            
    unique_channel= st.selectbox("Select the Channel",uniq_channel)
    
    if st.button("Migrate to Sql"):
        Table=tbl.tables(unique_channel)
        st.success(Table)
    
if select == "JSON and Table":
    ...
    if show_table=="CHANNELS":
        ...
        db=mh.client["Youtube_data"]
        coll1=db["channel_details"]
        for ch_data in coll1.find({},{"_id":0,"playlist_information":1}).limit(4):
